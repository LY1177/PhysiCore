...
async function loadTeacher() {
  ...
  try {
    const [overview, lb, users] = await Promise.all([
      api("/api/teacher/overview"),
      api("/api/teacher/leaderboard"),
      api("/api/teacher/users"),
    ]);
    ...
    const usersTbl = document.querySelector("#tblUsers");
    renderTable(
      usersTbl,
      ["ID", "Потребител", "Роля", "Създаден"],
      (users.users || []).map(u => [u.id, u.username, u.role, u.created_at])
    );
